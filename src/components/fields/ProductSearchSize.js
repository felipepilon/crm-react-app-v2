...
const ProductSearchSize = ({store_group_id, product_id, handleSizeSelect, size}) => {
    const [ sizes, setSizes ] = useState([]);

    const theme = useTheme();
    
    useEffect(() => {
        get_ProductSizeGrid({store_group_id, product_id})
        .then(res => {
            let i = 1;
            const newSizes = [];
            for(; i <= 50; i++)
                    ...
                    sizes.map((siz, i) => {
                        return (
                            <Button 
                                ...
                                onClick={(e) => _handleSizeSelect(e, size)}
                                variant='contained'
                                disableElevation
